...
function cmdShowDefaultIde(name) {
  const store = loadStore();

  if (!name) {
    if (store.ide_default) {
      console.log(`Global Default IDE: ${store.ide_default.toUpperCase()}`);
    } else {
      console.log("Global Default IDE: Not set");
    }
    return;
  }
  ...
  const localIde = repo ? repo.ide : collection.ide;
  const globalIde = store.ide_default;
  const effectiveIde = localIde || globalIde;

  const typeLabel = repo ? "Repo" : "Collection";

  console.log(`${typeLabel}: ${name}`);
  console.log(
    `  Local Preference:  ${localIde ? localIde.toUpperCase() : "Not set"}`
  );
  console.log(
    `  Global Default:    ${globalIde ? globalIde.toUpperCase() : "Not set"}`
  );
}
